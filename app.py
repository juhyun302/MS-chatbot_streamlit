...
with st.sidebar:
    ...
    st.markdown("""
        ...
        **1. 🎬 DTS 드라마와 배경 지식**
        - '본능의 질주'에 나오는 리카르도는 누구예요? (S1 이적 배경)
        - 샤를 르클레르는 왜 페라리에 그렇게 중요해요? (페라리의 신성)
        - 팀 보스들(크리스찬 호너, 토토 울프 등)은 왜 그렇게 감정적으로 싸워요?
        - 하스(Haas) 같은 약팀들은 왜 F1에 남아있어요? (재정 구조)
        - '본능의 질주' 드라마는 모두 실제 상황이에요? (극적 연출)
        - 막스 베르스타펜은 왜 초기 시즌에는 DTS 출연을 꺼렸어요?

        **2. 💰 드라이버의 압박 및 생활**
        - F1 드라이버가 되려면 얼마나 많은 돈이 필요해요? ('페이 드라이버' 개념)
        - 경기 중에 드라이버들은 서로 무전을 통해 무슨 이야기를 나누나요? (팀 오더)
        - 사고가 나면 드라이버들은 괜찮은 건가요? (안전 장치, HANS 장치)
        - 드라이버들은 레이스가 없는 날에는 주로 뭐해요? (체력 관리)

        **3. 🛠️ 경기 및 규칙 기초**
        - 그랑프리(GP)와 레이스 주말은 어떻게 진행돼요?
        - 퀄리파잉(Qualifying)은 뭐고, 왜 중요해요?
        - DRS(Drag Reduction System)는 정확히 언제, 어떻게 쓸 수 있어요?
        - F1 머신은 왜 이렇게 비행기처럼 생겼어요? (에어로다이내믹스)
        - 지금 제일 잘 나가는 팀과 선수는 누구예요? (최근 DTS 시즌 강자 파악)
    """)
    # ⭐⭐ 섹션 종료 ⭐⭐
